...
- GitHub Action：`linsk27/proof-pr@v0.1.49`
...
- **发布链路已验证**：npm Trusted Publishing 已绑定 `linsk27/proof-pr` + `release.yml`，`v0.1.21` 到 `v0.1.49` 已通过 GitHub OIDC 自动发布到 npm。
...
本地 `check` 通常没有 PR 标题和描述上下文，报告会明确提示这一点；打开 PR 后，ProofPR 会结合 PR 标题和描述重新评估证据。需要本地模拟 PR 描述时，可以把描述写入 `pr.md` 后运行 `npx proof-pr@latest check --pr-body-file pr.md`。
