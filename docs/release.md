...
`v0.1.51` 是命令收敛修正版，重点包括：

- 清理快速开始文档里的旧 `proof-pr scan` 示例，统一改成公开主命令 `proof-pr check`。
- CLI 在无法自动识别 base 分支时，只提示 `proof-pr check --base origin/main`，避免用户看到不存在或不推荐的命令入口。
- 发布文档同步说明：需要 PR 描述文件或特殊 base 时，仍优先使用 `check --base ... --pr-body-file ...`。

## `v0.1.50` 发布状态

`v0.1.50` 是 PR 描述文件入口可发现性增强版，重点包括：

- `check --help` 的常用复制命令补充 `npx proof-pr@latest check --pr-body-file pr.md`。
- `request --help` 的常用复制命令补充 `npx proof-pr@latest request --pr-body-file pr.md`。
- 默认参数列表仍保持简洁；README、命令文档、快速开始和 npm README 同步说明这些命令在需要本地模拟 PR 描述时使用。
...
- 文档和截图同步改成更短命令；需要 PR body 或特殊 base 时，仍优先使用 `check --base ... --pr-body-file ...`。
