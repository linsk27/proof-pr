...
2. 发布链路稳定性
   Trusted Publisher 已配置并通过 `v0.1.21`、`v0.1.22`、`v0.1.23` 验证。后续重点是保持 release workflow 简单可审计，避免回退到本地 token 发布。
...
下一步优先用 `0.1.23` 的聚焦定位和简化接入流程更新真实 demo PR 截图和 Marketplace 页面，并继续准备 GitHub Marketplace 上架材料。
