...
2. 发布链路稳定性
   Trusted Publisher 已配置并通过 `v0.1.21` 到 `v0.1.36` 验证。后续重点是保持 release workflow 简单可审计，避免回退到本地 token 发布。
...
下一步优先用 `0.1.36` 的中文 CLI 帮助和简化入口更新真实 demo PR 截图、Marketplace 页面和从 0 到 1 教程，并继续准备 GitHub Marketplace 上架材料。
