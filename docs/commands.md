...
- workflow 是否使用当前推荐的 `linsk-labs/proof-pr@v1.0.0`。
...
本地运行时通常没有 PR 标题和描述上下文，报告会明确提示这一点；打开 PR 后，ProofPR 会结合 PR 标题和描述重新评估证据。需要本地模拟 PR 描述时，可以把描述写入 `pr.md` 后运行：
