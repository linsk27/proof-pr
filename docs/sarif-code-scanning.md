...
```bash
npx proof-pr@latest scan --base origin/main --head HEAD --format sarif > proofpr.sarif
```
  ...
  proofpr-sarif:
    ...
    steps:
      ...
      - uses: actions/setup-node@v4
        with:
          node-version: 20
      - run: npx proof-pr@latest scan --base origin/main --head HEAD --format sarif > proofpr.sarif
