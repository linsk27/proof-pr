...
function renderChineseMarkdownReport(result: ScanResult): string {
  const lines: string[] = [
    ...
    `| 审查门禁 | **${formatReviewDecision(result.reviewDecision, "zh-CN")}** |`,
...
function appendRiskRadarSection(lines: string[], result: ScanResult, locale: ReportLocale): void {
  ...
  lines.push(
    locale === "zh-CN"
      ? "这部分把规则 ID 归并成维护者更容易理解的风险来源。"
      : "This groups rule ids into maintainer-facing risk sources."
  );
  lines.push("", locale === "zh-CN" ? "| 风险来源 | 发现数 | 最高级别 | 说明 |" : "| Source | Findings | Highest | Why it matters |");
  lines.push("| --- | ---: | --- | --- |");
...
function appendReviewPlanSection(lines: string[], result: ScanResult, locale: ReportLocale): void {
  lines.push(locale === "zh-CN" ? "## 审查行动清单" : "## Review Plan", "");
...
function translateSecretPatternLabel(ruleId: string): string {
...
function htmlLabels(locale: ReportLocale): HtmlLabels {
  if (locale === "zh-CN") {
    return {
      ...
      rulesCovered: "命中规则",
      noRules: "没有规则命中。",
      noFindings: "启用的规则没有发现需要优先关注的审查风险。",
      rule: "规则",
      severity: "严重程度",
      path: "路径",
      detail: "详情",
